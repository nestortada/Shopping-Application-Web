...
export default function NotificationItem({ notification }) {
  ...
  return (
    <li 
      ref={itemRef}
      className={`${itemClasses} transition-all duration-200`}
      style={{ 
        transform: `translateX(${currentX}px)`,
        opacity: 1 - (currentX / 200), // Fade out as it's swiped
        background: currentX > 50 ? 'rgba(254, 226, 226, 0.6)' : '', // Light red background when swiping
      }}
    ...
    >
      ...
      {currentX > 20 && (
        <div 
          className="absolute right-0 top-0 bottom-0 flex items-center pr-4 text-red-500 transition-opacity"
          style={{ 
            opacity: currentX / 100,
            transform: `scale(${Math.min(1, currentX / 80)})` // Scale up as user swipes
          }}
        >
